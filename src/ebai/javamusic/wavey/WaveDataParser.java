...
public class WaveDataParser {
	...
	public WaveDataParser(ArrayList<WaveDataPoint> waveData, boolean proc, boolean print)
	{
		...
		// in print mode, output data here and stop
		if (print) {
			System.out.println("Writing to chart_data.py and exiting");
			try {
				printPythonData();
			} catch (IOException e)
			{
	...
	private void printPythonData() throws IOException
	{
		// outputs data in matplotlib-friendly syntax
		// for debugging and fun times
		
		File f = new File("chart_data.py");
		...
		for (int i = 0; i < this.getChordA().size() - 1; i++)
		{
			pw.print(this.getChordA().get(i).getAmplitude() + ", ");
		}
		pw.println(this.getChordA().get(this.getChordA().size()-1).getAmplitude() + "]");
		
		// amplitude b
		pw.print("amplitude_b = [");
		for (int i = 0; i < this.getChordB().size() - 1; i++)
		{
			pw.print(this.getChordB().get(i).getAmplitude() + ", ");
		}
		pw.println(this.getChordB().get(this.getChordB().size()-1).getAmplitude() + "]");
		
		// arp period a
		pw.print("arp_period_a = [");
		for (int i = 0; i < this.getChordA().size() - 1; i++)
		{
			pw.print(this.getChordA().get(i).getArpPeriod() + ", ");
		}
		pw.println(this.getChordA().get(this.getChordA().size()-1).getArpPeriod() + "]");
		
		// arp period b
		pw.print("arp_period_b = [");
		for (int i = 0; i < this.getChordB().size() - 1; i++)
		{
			pw.print(this.getChordB().get(i).getArpPeriod() + ", ");
		}
		pw.println(this.getChordB().get(this.getChordB().size()-1).getArpPeriod() + "]");
